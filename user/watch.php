...
$stmt = $pdo->prepare("SELECT e.*, s.stream_key FROM events e LEFT JOIN streams s ON e.id = s.event_id WHERE e.id = ?");
$stmt->execute([$id]);
$event = $stmt->fetch();
            ...
            <div class="card-body p-0">
                <?php if($event['status'] == 'live' && !empty($event['stream_key'])): ?>
                    <div class="video-container">
                        <video id="stream-player" class="video-js vjs-default-skin vjs-big-play-centered" controls preload="auto" playsinline webkit-playsinline crossorigin="anonymous" data-setup='{"fluid": true}'>
                            <!-- Assuming Nginx RTMP HLS output is mapped to /hls/ -->
                            <source src="<?= e(HLS_BASE_URL) ?>/<?= e($event['stream_key']) ?>.m3u8" type="application/x-mpegURL">
                            <p class="vjs-no-js">
